...
class AsyncGeocoder(geo: Geocoder) {
  ...
  /** Perform an address lookup that returns a `scala.concurrent.Future[Result]`;
    ...
    */
  def lookup(address: String)(implicit context: ExecutionContext): FutureResult =
    Future {
      geo.lookup(address)
    }(context)
  ...
  /** Perform a component lookup that returns a `scala.concurrent.Future[Result]`.
    ...
    * @see [[com.koddi.geocoder.Geocoder]]
    */
  def lookup(components: Seq[AbstractComponent])(implicit context: ExecutionContext): FutureResult =
    Future {
  ...
  /** Perform an place id lookup that returns a `scala.concurrent.Future[Result]`;
    ...
    * @see [[com.koddi.geocoder.Geocoder]]
    */
  def lookupPlace(placeId: String)(implicit context: ExecutionContext): FutureResult =
    Future {
