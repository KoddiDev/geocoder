...
class Geocoder(
  ...
  def lookup(latitude: Double, longitude: Double): Seq[Result] = {
    sendRequest(
        Geocoder.API_PARAM_LATLNG,
        Location(latitude, longitude).toString()
    )
  }
  ...
  protected def sendRequest(searchParam: String, searchValue: String): Seq[Result] = {
    ...
    if (!response.success) {
      val message = response.errorMessage match {
        ...
        case None =>
          s"${searchParam.capitalize} '$searchValue' could not be geocoded."
      }

      response.status match {
        case Response.STATUS_OVER_QUERY_LIMIT =>
          throw OverQueryLimitException(message)
        case Response.STATUS_INVALID_REQUEST =>
          throw InvalidRequestException(message)
        case status => throw FailedResponseException(status, message)
      }
    }

    if (response.hasNoResults) Seq.empty[Result] else response.results
  }
  ...
  protected def doGetRequest(url: URL): Response = {
    ...
    try {
      ...
      case e: Exception => throw ResponseParsingException(e)
    } finally {
