...
class AsyncGeocoderSpec extends TestSpec {
  ...
  "An AsyncGeocoder" should "lookup a given address and trigger a success" in {
    val geocoder =
      new AsyncGeocoder(new MockGeocoder("api_response_address.xml"))
  ...
  it should "reverse lookup lat/lng values and trigger a success" in {
    val geocoder =
      new AsyncGeocoder(new MockGeocoder("api_response_latlng.xml"))
  ...
  it should "lookup an address by component objects" in {
    val geocoder =
      new AsyncGeocoder(new MockGeocoder("api_response_address.xml"))
    val query =
      geocoder.lookup(Seq(PostalCodeComponent("76107"), CountryComponent("us")))
  ...
  it should "return an empty result sequence when the response has zero results" in {
    val geocoder =
      new AsyncGeocoder(new MockGeocoder("api_response_zero_results.xml"))
  ...
  it should "trigger a failure when an invalid lat/lng is given" in {
    val geocoder =
      new AsyncGeocoder(new MockGeocoder("api_response_invalid.xml"))
